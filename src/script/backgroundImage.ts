...
export const getLinkToImage: () => Promise<void> = async () => {
  const img = new Image();
  const url = data.state.photoSource === 'unsplash'
  ...
  img.src = data.state.photoSource === 'unsplash'
...
export const changeSlideApi: () => void = () => {
